...
> **用途边界。** 本合同是本轮 MVP 开发的**外部验收方法**，用于团队判断交付是否达标；不是 MVP 产品向终端用户暴露的运行时功能。当前锁定的 MVP 产品范围（1 个 Epic、2 条 Story：查看/比较配置、选择配置并使用 OMP）由 [`epics.md`](../../planning-artifacts/epics.md) 权威定义，见其 AR15 对三层验证/样本门的裁决。
